...
describe('check compare', () => {
  test.each(data2)('compare test nested %s, %s, %s', (firstFile, secondFile, stylishFile) => {
    const expectedResult = readFixture(stylishFile).trim();
    const actualResult = genDiff(getFixturePath(firstFile), getFixturePath(secondFile));
